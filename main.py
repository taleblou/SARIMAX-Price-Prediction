...
def build_model(X_train,Y_train):
    ...
    # Generate combinations of non-seasonal parameters (p, d, q)
    non_seasonal_params = list(product(range(2), range(2), range(2)))

    # Generate combinations of seasonal parameters (P, D, Q)
    # seasonal_params = list(product(range(2), range(2), range(2)))

    for non_seasonal in non_seasonal_params:
        # for seasonal in seasonal_params:
            try:
                # Fit SARIMA model
                model = SARIMAX(X_train, order=non_seasonal, seasonal_order=(1, 1, 1, 12))
                model_fit = model.fit(disp=False)

                # Evaluate model performance using RMSE
                predictions = model_fit.fittedvalues
                score = np.sqrt(mean_squared_error(Y_train, predictions))

                # Store the best model and parameters
                if score < best_score:
                    best_score = score
                    best_order = non_seasonal
...
for i in tqdm(range(box-1)):
    ...
    for c in ['Open','High','Low','Close']:
        ...
        future_exog = Y_test["y_" + c].values.reshape(-1, 1)
        predictions = model.forecast(steps=len(Y_test.index), exog=future_exog)
...
for c in ['Open','High','Low','Close']:
    ...
    plt.tight_layout()
    # plt.show()
    plt.savefig(f"SARIMAX_{c}_{name}.png")
    plt.close()
    plt.cla()
